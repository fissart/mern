...
sgMail.setApiKey(
  "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"
);
////////////////////////////////////////////////////////
...
exports.signinController = async (req, res) => {
  ...
  if (!errors.isEmpty()) {
    ...
    User.findOne({
      ...
      if (err || !user) {
        return res.status(400).json({
          errors: "Ususario con el email no existe. Por favor regístrese",
        });
      }
      // authenticate
      if (user.password !== password) {
        return res.status(400).json({
          errors: "Email y password no coinciden",
        });
      }
      ...
      const { _id, name, email, role, foto } = user;
